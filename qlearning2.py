...
def value_iteration(P, n_states, n_actions, gamma=0.99, theta=1e-10):
    """
    ...
    # Identify terminal states (where done=True for all actions)
    terminal_states = set()
    for s in range(n_states):
        if s in P and all(
            all(done for _, _, _, done in P[s][a]) for a in range(n_actions) if a in P[s]
        ):
    ...
    for iteration in range(10000):
        ...
        for s in range(n_states):
            if s in terminal_states:
                # Terminal states have Q = 0 (no future actions possible)
                continue
...
def q_learning(env, n_episodes, gamma=0.99, alpha_mode="decay",
    ...
    for ep in range(n_episodes):
        ...
        while not done and steps < 500:
            ...
            # Learning rate
            if alpha_mode == "decay":
                # 1/n decays too fast in practice. Use 1/n^0.6 which still
                # satisfies the theorem conditions (sum diverges, sum of
                # squares converges) but decays slowly enough to learn.
                alpha = 1.0 / (visit_count[state, action] ** 0.6)
        ...
        if Q_star is not None:
            ...
            # The agent never acts from a terminal state, so Q-values
            # there are never updated and comparing them is meaningless.
            ...
            learned_policy = np.argmax(Q[non_terminal], axis=1)
            optimal_policy = np.argmax(Q_star[non_terminal], axis=1)
            policy_matches.append(np.mean(learned_policy == optimal_policy))
...
def experiment_1_convergence():
    ...
    ax1.plot(episodes, mean_error, color='blue', linewidth=0.5)
    ax1.fill_between(episodes, mean_error - std_error, mean_error + std_error,
                     alpha=0.2, color='blue')
    ax1.set_xlabel('Episode')
    ax1.set_ylabel('||Q_n - Q*||∞')
    ax1.set_title('Q-Value Convergence Error (4x4 GridWorld)')
    ax1.set_yscale('log')
    ...
    print(f"\nFinal error (mean ± std): {mean_error[-1]:.6f} ± {std_error[-1]:.6f}")
...
if __name__ == "__main__":
    ...
    Q_learned, _ = q_learning(
        env, n_episodes=20000, gamma=0.99,
        alpha_mode="decay", epsilon_mode="decay", Q_star=Q_star
    )
