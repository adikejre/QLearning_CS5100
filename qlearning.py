...
def q_learning(env, n_episodes, gamma=0.99, alpha_mode="decay",
    ...
    for ep in range(n_episodes):
        ...
        if Q_star is not None:
            q_errors.append(np.max(np.abs(Q - Q_star)))
...
def double_q_learning(env, n_episodes, gamma=0.99, alpha_mode="decay",
    ...
    for ep in range(n_episodes):
        ...
        if Q_star is not None:
            ...
            q_errors.append(np.max(np.abs(Q_avg - Q_star)))
            overestimation.append(np.mean(Q_avg - Q_star))
...
def experiment_2_learning_rates():
    ...
    - α = 1/n satisfies both → should converge to Q*
    - α = 0.1 fixed → Σα² = ∞, violates condition → should oscillate
    - α = 0.5 fixed → same violation, more aggressive oscillation
    """
    print("=" * 60)
    ...
    configs = [
        ("α = 1/n (decaying)", "decay", 0.0),
        ("α = 0.1 (fixed)", "fixed", 0.1),
        ("α = 0.5 (fixed)", "fixed", 0.5),
    ]
